...
class HocKy(Base_CLS):
    def __init__(self,MaHocKy = '', TenHocKy = '',HeSo='') -> None:
    ...
    def __display_data(self):
        sql_cmd = "SELECT * FROM HocKy"
        self.__db_cur.execute(sql_cmd)
        results = self.__db_cur.fetchall()
        
        list_st=[]
        for idx,row in enumerate(results):
            st_if=[row[0],row[1],row[2]]
            list_st.append(st_if)
        print(tabulate(list_st, headers=["MaHocKy","TenHocKy","HeSo"],numalign="left"))
